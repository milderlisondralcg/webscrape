...
/**
* calculate-delta.php
* Calculate the delta for all records within productsdelta table
*
**/
...
$error_log_file = create_log_file('calculate-delta');
record_log_message(date("Y-m-d h:i:s A") . " Calculate Delta Started");
...
record_log_message(date("Y-m-d h:i:s A") . " Calculate Delta Completed");
class DB{
	...
	public function get_products($query_limit){ 
		...
		$select_query = "SELECT id, catalog_id, date_20220307,
       date_20220309,
       date_20220310,
       date_20220311,
       date_20220314,
       ...
       date_20221212,
       date_20221219,
       date_20221229 from ".$this->inventory_delta." WHERE delta_status IS NULL LIMIT " . $query_limit;

		//$results = R::getAll("SELECT * from " . $this->inventory_delta . " WHERE delta_status IS NULL LIMIT " . $query_limit);
		
		$results = R::getAll($select_query);

		foreach($results as $result){

			print "Catalog ID: " .  $result["catalog_id"] . PHP_EOL;
			print "ID: " . $result["id"] . PHP_EOL;
			$record_id =  $result["id"];
			$inventory_arr = [];
			...
			foreach($inventory_arr as $inventory){
				...
				if($inventory > $cur_inventory){
					...
					$cur_inventory = $inventory; // replace with the larger "inventory value"
				}
				//print "Current Inventory: " . $cur_inventory . PHP_EOL;

				if($count == count($inventory_arr)){
			...
			// Update delta for given record
			$update_str = "UPDATE " . $this->inventory_delta . " SET delta = '".$delta."', delta_status = 'Done' WHERE id = '".$record_id."';";

			//print $update_str . PHP_EOL;
			R::exec( $update_str );
